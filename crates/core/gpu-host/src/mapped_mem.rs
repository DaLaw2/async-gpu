...
/// Allocate a byte buffer in pinned, device-mapped host memory.
///
/// # Safety
/// Caller must free the returned host pointer via [`free_mapped_bytes`].
/// The CUDA context must be initialized.
pub unsafe fn alloc_mapped_bytes(
    _dev: &Arc<CudaDevice>,
    size: usize,
) -> Result<(*mut u8, sys::CUdeviceptr)> {
    let cu = cuda_lib();
    ...
    std::ptr::write_bytes(host_ptr as *mut u8, 0, size);
    Ok((host_ptr as *mut u8, dev_ptr))
}

/// Free a mapped byte buffer.
///
/// # Safety
/// `host_ptr` must have been returned by [`alloc_mapped_bytes`] and not yet freed.
pub unsafe fn free_mapped_bytes(host_ptr: *mut u8) -> Result<()> {
    let cu = cuda_lib();
    let result = cu.cuMemFreeHost(host_ptr as *mut std::ffi::c_void);
    if result != sys::CUresult::CUDA_SUCCESS {
        return Err(GpuHostError::CudaFreeMem(result));
