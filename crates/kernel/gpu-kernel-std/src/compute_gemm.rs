...
// ============================================================
// High-performance f32 GEMM v3: 128×128 tile, 8×8 register blocking
// ============================================================

/// High-performance GEMM v3: D = A × B (both row-major).
///
/// 128×128 CTA tile, BK=8, 256 threads.
/// Each thread computes 8×8 = 64 output elements (4:1 FMA:load ratio).
/// Thread grid: 16×16 over the 128×128 tile.
///
...
pub unsafe extern "gpu-kernel" fn gemm_f32_v3(
    ...
    #[cfg(target_arch = "nvptx64")]
    {
        ...
        const BN: u32 = 128;
        const BK: u32 = 8;
        const A_STRIDE: u32 = BM + 4; // 132
        const STAGE_FLOATS: u32 = BK * A_STRIDE + BK * BN; // 8*132+8*128 = 2080

        let smem = get_dynamic_smem_ptr() as *mut f32;

        // 256 threads as 16×16, each computes 8×8
        ...
        while t < k_tiles {
            ...
            let bs_ = (buf * STAGE_FLOATS + BK * A_STRIDE) as usize;
            let arb = (thread_row * 8) as usize;
            let bcb = (thread_col * 8) as usize;
    ...
    if tid == 0 {
        let _prev: u32;
        core::arch::asm!("atom.global.add.u32 {prev}, [{addr}], 1;", prev = out(reg32) _prev, addr = in(reg64) status);
    }
}

/// Load A+B tile for v3 (128×128, BK=8).
#[inline(always)]
#[allow(clippy::too_many_arguments)]
