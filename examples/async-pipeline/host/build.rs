...
fn main() {
    ...
    // Look for the patched rustc sysroot
    ...
    // Use stock nightly toolchain for cargo itself
    let toolchain = read_nightly_toolchain(&repo_root);
    cmd.arg(&toolchain);
    cmd.args(["build", "--release"]);
    cmd.current_dir(&kernel_dir);

    // Clean environment to avoid inheriting host build settings
    cmd.env_remove("CARGO_ENCODED_RUSTFLAGS");
    cmd.env_remove("CARGO_TARGET_DIR");
    cmd.env_remove("CARGO_BUILD_TARGET");

    if use_patched {
        // Point to patched rustc for the kernel compilation
        cmd.env("RUSTC", &patched_rustc);
    } else {
        cmd.env_remove("RUSTC");
    }
    cmd.env_remove("RUSTFLAGS");
    // Remove CARGO to avoid cargo recursion issues
    ...
    if !status.success() {
        let ptx_fallback = kernel_dir
            ...
            .join("release")
            .join("async_pipeline_kernel.ptx");
    ...
    // Patch PTX target version if needed (sm_30 → sm_86)
