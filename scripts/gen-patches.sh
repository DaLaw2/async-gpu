...
# Regenerates scripts/apply-patches.sh and std-patches/PATCHES.md automatically.
...
# --- Generate apply-patches.sh ---
cat > "$SCRIPT_DIR/apply-patches.sh" << 'HEADER'
#!/bin/bash
# Apply std patches for nvptx64 GPU compilation.
# Auto-generated by gen-patches.sh — do not edit manually.
#
# Usage: ./scripts/apply-patches.sh [output_dir]
...
for entry in "${PATCHES[@]}"; do
    rel_path="${entry%%:*}"
    patch_name="${entry##*:}"
    cat >> "$SCRIPT_DIR/apply-patches.sh" << EOF
echo "  [PATCH] $rel_path"
...
cat >> "$SCRIPT_DIR/apply-patches.sh" << 'FOOTER'
echo ""
...
chmod +x "$SCRIPT_DIR/apply-patches.sh"
...
echo ""
echo "Updated: scripts/apply-patches.sh, std-patches/PATCHES.md"
echo "Done!"
