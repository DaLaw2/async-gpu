...
#
# Usage: ./std-patches/gen-patches.sh
#
...
# Regenerates apply.sh and PATCHES.md automatically.
...
# Clean old generated files (keep this script)
cd "$SCRIPT_DIR"
rm -f *.patch *.rs
...
while IFS= read -r patched_file; do
    ...
    if [ ! -f "$sysroot_file" ]; then
        ...
        if ! diff -q "$sysroot_file" "$patched_file" > /dev/null 2>&1; then
            ...
            # Generate patch with paths relative to library/ so patch -p0 works from there
            diff -u "std/${patched_file#$PATCHED_STD/}" "std/${patched_file#$PATCHED_STD/}" \
                --label "a/$rel_path" --label "b/$rel_path" < /dev/null > /dev/null 2>&1 || true
            # Actually generate the diff with proper labels
            diff -u "$sysroot_file" "$patched_file" \
                --label "$rel_path.orig" --label "$rel_path" \
                > "$SCRIPT_DIR/$patch_name" || true
...
# --- Generate apply.sh ---
cat > "$SCRIPT_DIR/apply.sh" << 'HEADER'
#!/bin/bash
...
for entry in "${PATCHES[@]}"; do
    rel_path="${entry%%:*}"
    patch_name="${entry##*:}"
    cat >> "$SCRIPT_DIR/apply.sh" << EOF
echo "  [PATCH] $rel_path"
patch -p0 --binary < "\$SCRIPT_DIR/$patch_name"
...
echo "  [NEW]   $rel_path"
mkdir -p "$dir_path"
cp "\$SCRIPT_DIR/$flat_name" "$rel_path"

EOF
done

cat >> "$SCRIPT_DIR/apply.sh" << 'FOOTER'
echo ""
...
for entry in "${PATCHES[@]}"; do
    rel_path="${entry%%:*}"
    patch_name="${entry##*:}"
    echo "- \`$rel_path\` → \`std-patches/$patch_name\`" >> "$SCRIPT_DIR/PATCHES.md"
done

echo "" >> "$SCRIPT_DIR/PATCHES.md"
echo "## New files" >> "$SCRIPT_DIR/PATCHES.md"
echo "" >> "$SCRIPT_DIR/PATCHES.md"

for entry in "${NEW_FILES[@]}"; do
    rel_path="${entry%%:*}"
    flat_name="${entry##*:}"
    echo "- \`$rel_path\` → \`std-patches/$flat_name\`" >> "$SCRIPT_DIR/PATCHES.md"
done

echo ""
echo "Updated: apply.sh, PATCHES.md"
echo "Done!"
